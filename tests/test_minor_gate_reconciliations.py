...
def test_minor_versions_on_the_1_2_0_line():
    # D4: the milestone version numbers are current
    assert _read("VERSION").strip() == "1.2.0"
    assert '"version": "1.2.0"' in _read(".claude-plugin/plugin.json")
    assert "version: 1.1.0" in _read("skills/product-prover/SKILL.md")
    assert "v1.2.0, 2026-07-13" in _read("PRODUCT_SPEC.md")
