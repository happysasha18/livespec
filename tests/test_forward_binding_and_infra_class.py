...
def test_forward_binding_law_stated_once():
    spec = _spec()
    # the law is stated in its own sentence, in words, tagged INV-159
...
def test_forward_binding_cites_are_repointed_off_the_silent_roots():
    ...
    # and the repointed cites resolve to INV-159 instead
    assert "binds forward [INV-159]" in spec
    assert "forward-binding intake law [INV-159]" in spec
...
def test_infra_class_enumerates_every_member():
    # the class clause must name each of the nine members, so an author adding a tenth
    # has an enumerated parity to write against (the gap the design review flagged)
...
def test_infra_class_states_net_parity_and_binds_forward():
    ...
    assert "names the net" in body
    # a member with no net is the prover's declared-class defect
    assert "[INV-125]" in body
