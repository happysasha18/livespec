...
| M-022 | One landing at a time per repo under one pen; at most TWO rows in-work at once (the two-lane cap, T-18) and claiming stays an atomic committed flip; never a third row in-work, never two foreign sessions sharing a repo's pen | INV-2 | string | `test_roadmap_in_work_cap` | BUILT |
| M-129 | Parallel lanes: one session may roll at most two INDEPENDENT build lanes (T-18's [default] cap) — opening narrated, both trains on the departures board, the waiting lane naming whom it waits behind; penless stages overlap (the second train's code and tests in an isolated tree only; disjoint-file workers within one lane; read-only analysis free); pen-stages serialize (every shared doc — spec, architecture, matrix, queue, journal, resume file — plus integration and row close), a pen-stage never cut mid-edit; never a third build lane, never parallel lanes across sessions, never mid-milestone, never a spec-delta proven against another lane's half-written draft | T-18 | string | `test_parallel_lanes_law` | BUILT |
| M-130 | Landing purity under two trains: a landing commit carries exactly one row's delta and its gate (full suite + guardrails) runs on a tree holding nothing of any other lane's unfinished work; after a landing the waiting lane re-checks the fence and re-runs its gate on the new truth — landed-first wins; never half of another train riding a landing, never the second lane landing unre-verified | INV-39 | string | `test_landing_purity` | BUILT |
...
| M-124 | Working narration: while work runs, beats are narrated as they happen — a pipeline station passed, a load-bearing find, a change of direction — one-two plain sentences in the roadmap's terms, the reports' voice, the mechanical grind quiet; a narration line is chat, not a report — it walks no pre-report walk, asks no questions, and the plain-language + bookkeeping laws still bind it; never a beat-worth stretch left silent between echo and report, never narration replacing the milestone report or triggering the walk | INV-35 | string | `test_working_narration` | BUILT |
