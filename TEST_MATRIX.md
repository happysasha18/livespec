...
| M-112 | The departures board: every status report names each in-flight feature by its echo-name with its pipeline station (spec → prove → architecture → matrix → tests → code → verify → landed), the station vocabulary being the pipeline's own step names; never progress reported as prose archaeology, never a station name drifting from the pipeline's step names | INV-27 | string | `test_capture_echo_and_board` | BUILT |
...
| M-116 | A taste default is a loan: every landing report lists the open `[default]`s with their age in landings; one surviving two landings without the human's word rides the next decision page flagged loudly; never a default ageing silently past two landings, never a list dropped between reports | INV-31 | string | `test_default_expiry_law` | BUILT |
