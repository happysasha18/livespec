...
| M-029 | The milestone gate runs: full re-prove + matrix audit + composition check + compaction + gate/inbox re-listing + index re-check + the loader-stays-thin walk (line count stated; a line failing the must-hold-before-pack-loads test migrates to its real home); never a MINOR without it | M-1 | string | clause presence: `test_m1_names_loader_thin_item`; the walk itself: next MINOR walks it; journal records the walk (first walk journaled 2026-07-05, session 9: 16 non-empty lines, all pass) | TODO |
