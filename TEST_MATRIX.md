...
| M-136 | The architecture owes numbers: for every user-facing surface the architecture doc states measurable quality budgets (performance first) plus an instrumentation home (timings measured and human-readable), each budget asserted by a matrix-row acceptance; spec-author's performance facet ends as a budget sentence and build-pipeline's architecture step demands the budget line; never a user-facing surface passing derivation with no budget and no instrumentation home, never a budget living only as prose hope | INV-41 | string | `test_architecture_owes_budgets` | BUILT |
