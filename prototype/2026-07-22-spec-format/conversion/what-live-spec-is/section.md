...
Terms already defined in the intake glossary and the founding section — request, pipeline, spec, test matrix, queue, delivery, journal, inbox, host, pack, session, profile, adoption, guardrail, surface registry, feedback ledger, snapshot, design-sync, skill eval — carry their meanings unchanged. The block below adds only the new nouns this section needs.
...
- **checkpoints** — the saved points a lane's work reaches and can resume from, written under `.live-spec/`.

---

## Requirement 1: The spec keeps what is built apart from what is planned

**Context:** The spec states what is built and working today apart from what is only planned, and it keeps a reader from mistaking one for the other. A planned feature carries the target tag on a line of its own, and the tag never spreads to the section around it. The suite ties each target tag to the open roadmap row that builds it, so the marker is enforced rather than trusted.
...
1. The spec *shall* state what is built and working today apart from what is only planned, marking each scenario and its named promised parts apart, so a scenario holding both built and planned law states each part's status at that same level. [S-0]
...
3. The system *shall* tie each target tag to the open roadmap row that builds it, and *shall* red the suite *if* that row ships with the tag still on, *if* the tag vanishes, or *if* the tag was never named. [S-0]
4. The system *shall* mark as planned the host-facing guardrail checks and the surface registry, the snapshot machinery the adoption baseline uses, and the design-sync machine. [E-6, E-10, E-7, A-6, E-18]

---

## Requirement 2: The pipeline runs as a set of roles carried by the working skills

**Context:** Behind the pipeline is a full set of roles. An analyst writes the spec, an architect stress-tests the design and finds the edge cases and dead ends before any code is written, a design reviewer judges the design and checks that same-kind things behave alike, a tester works out the tests and writes them, and a project manager runs the process and reports back to the person. These roles are the working skills, and one base skill holds the shared rulebook and the default settings the other skills work by.
...
1. The system *shall* run each request through a set of roles — an analyst who writes the spec, an architect who finds the edge cases and dead ends before any code, a design reviewer who checks that same-kind things behave alike, a tester who works out and writes the tests, and a project manager who runs the process and reports back. [E-12]
...
1. The system *shall* let a project adopt live-spec at the start or partway through work already under way, bringing the document templates, a procedure for joining midstream, and the guardrails the project installs. [E-1]
