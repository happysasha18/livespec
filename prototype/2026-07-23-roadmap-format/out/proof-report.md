...
Compared the OLD `ROADMAP.md` body against the NEW live body plus the July archive, over the data-row region (preambles, table headers, and separators excluded on both sides), word-token multiset and punctuation multiset, modulo the named deltas below. Live rows: 117 (queued 82 · in-work 3 · deferred 29 · far 3). Archived rows: 228 — moved verbatim, byte-identical, so they cancel.

## Named deltas (excluded regions, reported)

- **Preamble replaced** — excluded both sides. Old preamble+manifest 212 word tokens; new 537 word tokens (the new-format preamble; the manifest keeps the 2026-07-18 line and gains one July line).
...
- **Status normalized, token-preserving** — the old status cell of each of the 117 live rows is preserved unedited inside the wish cell's `(status note: …)`; ADDED per row: the wrapper (`status`, `note`; `(` `)` `:`) and the new `*word* DATE` status cell (a deferred row also `— revisit trigger: see the status note`).
