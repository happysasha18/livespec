...
7. **The concurrent-edit fence, before every write and every commit.** Re-check `git status` + HEAD against
   ...
   - **Three lanes under one pen.** Within ONE session up to three build lanes may roll without asking (SPEC T-18; a fourth only on the human's asked word): every write to a document the lanes share serializes under the single PEN, one lane at a time.
