...
> need primary sources · fix the class, sweep look-alikes · the door before code · prototype ≠ product) live ONCE in the pack's base skill, `live-spec-base` (v2.0.0), together with the
...
Read the kind from the finding's own ground: a broken or missing invariant or a false claim is a defect; a finding standing only on "these siblings should match" or "this could read clearer", with no invariant behind it, is a recommendation. The paired-transition family reads by the same split, said here once because two stations share the gap: a declared one-sided pair — one direction stated, the other silent [INV-126] — is this prover's finding and takes `defect` (a required answer is missing), while a never-declared same-kind grouping over the same physical gap belongs to the design review's motion-parity lens [INV-165] and recommends, never blocks. An open motion question — the undecidable pair surfaced to the human because motion feel is his gate — rides the batched-question path and does not hold the push: the pair lands under the stated default (symmetry), `[default]`-tagged until his word, the ordinary proceed-on-recommended arm (SPEC INV-4, INV-30, INV-31). Production impact is the reasoning behind that call — an atomicity gap on an automated path run thousands of times a day is a defect, the same gap on a manual quarterly operation a recommendation — and it belongs in the finding's consequence, not in a tag token. A Phase 3.5 acknowledged gap keeps its `acknowledged` tag and carries no kind — it is the document's own known issue, not a new finding. (SPEC INV-140)
