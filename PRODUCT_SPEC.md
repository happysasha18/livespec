# live-spec — Product Spec (v0.16.11, 2026-07-09)
...
| INV-75 | the placement view — THE tiers-and-technology table: every node states its place (build-time on the author's machine · CDN static · client browser · edge worker · external service) plus the load-bearing technology choice where one exists; first-class and at a glance; the doc reads tiers-first (opens with the shape at a glance, then nodes, then flows through the tiers); scales by kind [INV-36] — a book's one sentence satisfies it; binds forward like INV-74 [INV-15] | From the spec to the tests |
