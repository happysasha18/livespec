...
| INV-172 | adoption wires the ratchet gates in one pass: vendored scripts with source pins (pack version + content hash), debt caps seeded at the host's current measured size (green at once, ratchets down only), a generated guard test pinning the seeded caps, the push-gate wiring, all under the four-checks shipping contract [INV-97, INV-163, INV-164, INV-98] | Composing across axes |
