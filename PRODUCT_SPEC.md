# live-spec — Product Spec (v1.8.0, 2026-07-15)
...
These roles are real: they are the working skills (spec-author, product-prover, design-reviewer, build-pipeline, test-author, communicator, publish, feedback-intake). One **base skill** holds the shared rulebook and the default settings the other skills work by [E-12].
...
**The removal list has a mechanical form.** For a host with taste-reviewed artifacts, the pack ships a removal-list template. It holds the human's cuts as dated literals, appended the moment a cut happens and never removed. The pack also ships guardrails guidance for a scanner: a test reads the table and greps the artifact's surfaces, and a removed literal reappearing turns the suite red. The rule stays INV-42's; this is its enforcement. The scanner stays per-project by design, the same split the pack takes wherever a body is host-specific [INV-125, INV-136, INV-139]: the pack ships the shape and the host owns the instance. The discriminator is whether the pack can ship one identical body every host runs. The browser harness can, so it centralizes to one home [INV-158]. The removal-list scanner cannot: its greps name a host's own artifact surfaces and its table holds that host's own dated cuts, so the pack ships the shape — the template and the scanner guidance above — while each host owns the greps that read its own surfaces. A host whose scanner grows a genuinely generic seam lifts that seam to the pack; the host-specific greps stay home. [E-26]
...
| INV-101 | the spec names its cross-cutting laws in one declared-laws home (measurement, accessibility, error handling, register — what the product declares); every new surface's section states its line against each declared law (the clause or a dated exemption) before the prover reads it; each declared law also names its net — the enforcing review or gate — recorded beside it in this same home [INV-150]; the prover's station: per declared law, enumerate every surface and transition, demand the clause or dated exemption per item and a named net per law — a missing clause or a missing net ranks as a broken invariant; the pack's own list: register + clock-honesty + no-self-certification, two dated exemptions, all three gated mechanically (named guardrail scripts and tests) | Throwing a wish |
| INV-150 | every declared cross-cutting law names its enforcing net — one of three: a mechanical gate (a named guardrail script or a dedicated test, deterministic, blocks in CI), the prover's judgment station (a violation pins to a stated sentence — a clause, a declared law, a declared class [INV-125], a facet [INV-126] — and the walk blocks), or the design review's recommendation (the deciding fact lives only in the human's intent, soft, never blocking [INV-141]); the net is recorded beside the law in the declared-laws home [INV-101] and the station there demands one per law, a netless law ranking a broken invariant, a watch-level law naming the design review with a dated reason (the shape of a dated exemption); declaration is the promotion lever — an undeclared grouping is the design review's, a declared one the prover's (or a mechanical gate's where a deterministic check decides it), so a property is owned by no more than one net and dropped by none; the architecture's every-fact-owned-once check is the backstop | Throwing a wish |
| INV-151 | a request enters at the highest document in the derivation chain (spec → architecture → matrix → code → docs) whose sentences must change to satisfy it, the settings ladder standing beside the chain for a pure value [E-13]; the door set is CLOSED — product behaviour → the spec and flow down, a technically-phrased request → the architecture with the spec-motion back-check firing at intake (a surface/state/unbacked-behaviour trip lifts it to the spec at the door, outranking the label [INV-16], and re-firing mid-work), a defect → the matrix with a red-on-bug test, docs-only → its own light path, a tiny reversible edit → the skip shortcut owing the spec-backed-literal tripwire [INV-104], a settings/parameter value → the settings ladder [E-13], an outsider's request → one inbox wish [INV-17], a method/skill change → the same criterion with work-kind skill [INV-22] (no separate meta-layer set, the pack's product being the method), a see/try → a labelled sketch [E-17], research or a doc question → no layer, a hand-back → feedback-intake [T-20]; a request matching no kind is one plain question [INV-4], never a guessed route, the closed set making "no gap between the layers" checkable through the landing's applied-or-stood-down-by-name contract [INV-22]; homes — the entry-layer clause + build-pipeline's door step | Throwing a wish |
| INV-153 | the unification: the request classifier [INV-151], the property net [INV-150], and the deferral test [INV-152] are one principle — every incoming thing (a request, a property, a work item) routes to the home whose declared sentence governs it, and a thing that pins to no home is itself the finding (a request matching no kind → one plain question [INV-4], a netless declared law → a broken invariant [INV-101], a held work item that cannot name its human-only fact → the seat [INV-143]); declaration is the lever in all three; they stay separate controls under one principle, verified each landing at no new machinery — the classifier by the applied-or-stood-down contract [INV-22], the property net by the declared-laws station [INV-101], the deferral by the derive-before-defer posture [INV-143]; homes — this unification clause + build-pipeline's door step | Throwing a wish |
| INV-104 | the bug door and the skip door carry one added door-step tripwire: does this edit touch a spec-backed literal or clause (version string, pinned count, named vocabulary, promised wording)? a yes binds the docs-travel-with-the-change rule and the red-first small-fix path into one duty — the docs and the test land in the same session as the fix; the tripwire reads the edit's content, so a one-word change to a spec-cited literal owes the same duty as a full feature | Throwing a wish |
...
| INV-144 | when the product and the spec diverge, the spec is the definition of correct, and a divergence defaults to a possible error in the product checked against the spec; the divergence is first named — what the spec states, what the product does, why they differ — and routed to the home that owns it [INV-37]; a product that is wrong is fixed to the spec [INV-124], a spec silent where the product is correct is completed to state the guarantee with a test pinning it (the completion rides the ordinary spec-delta road as a default [INV-18, INV-31] unless what counts as correct is itself genuinely open, which only the human's word settles [INV-9]), a spec that conflicts with a correct product is changed only when the spec is confirmed the error and the human has understood the divergence and given their word [INV-4, INV-9]; the spec is never silently rewritten to match the product; homes — this reconciliation clause + product-prover's divergence-finding pointer | Throwing a wish |
...
| INV-133 | a critical non-bug heads the queue but never preempts a rolling lane, and the bound is echoed back at intake: "critical" is defined in bug terms but liftable onto any door [T-11], so a critical non-bug could head the queue and then wait for the current lane while a human who said "critical" for a live break expected bug-like preemption; preemption belongs to the bug door alone [T-9], so a critical non-bug is admitted at the current pen-holder's next pen-stage boundary — that lane's own landing when a single lane holds the pen straight through — ahead of everything else waiting but never interrupting the rolling lane's own stages, a genuine live break that must stop the work now being a bug; the capture echo [INV-27] says the bound back when a wish is marked critical on a non-bug door (it heads the queue, does not stop the lane, only the bug door preempts), so the human hears the difference and can re-door it a bug rather than discovering at the next report that the work kept running; the human's word still owns priority [INV-9], the clause stating what critical buys on each door and never refusing the mark; homes — the priority clause + communicator's capture echo; prover finding F5, 2026-07-12 | Throwing a wish |
...
| INV-136 | a project kind also carries its own design principles — checkable design rules the kind's products must hold, of the family of cross-surface policy uniformity [INV-125] and paired-transition symmetry [INV-126] — beside its concrete layers and proofs [INV-135]; the pack ships a per-kind starter set homed in the ARCHITECTURE per-kind design-principles table beside the footprint-and-proof scaffold, a founding that records a visual kind declares them in the host profile on a `project.design-principles` line, and a founding check reds a visual kind recorded with none the way a kind recorded with no layers or proofs is flagged [INV-135]; the verify pass reads the declared design principles and runs each in the medium's own form beside the visitor walk and feel pass [INV-30] — a principle the suite cannot green is the human's eye-walk [INV-77], one it can hold becomes a matrix row in the adopting project's suite; the frontend kind's starter set gathers the frontend guidance the pack already carries (the visitor walk · the feel pass scaled to a whole site · motion and scroll feel as the human's gate [INV-30]) plus its founding design principle, the interactive-overlap rule — interactive controls that belong to different layers occupy separate screen space (two interactive controls from different visual layers hold separate clickable regions so every press lands on one alone; a non-interactive element may overlap freely, the rule binding the clickable regions), its pixel/DOM projection living in the adopting project (live-spec has no UI, shipping the law + the starter set and leaving the pixel assertion to the products it serves, the split [INV-125] takes); homes — this founding clause + the ARCHITECTURE per-kind design-principles table + the host-profile record + spec-author + build-pipeline's verify feel pass + the founding check + product-prover's interactive-overlap lens; row 298 | Throwing a wish |
| INV-139 | the frontend kind carries a legibility floor beside the interactive-overlap rule [INV-136]: text meets a minimum contrast ratio against its background and a minimum size so a human can read it; two homes at the instant the register lint guards [INV-83] — the verify feel pass reads a product surface's computed colours and sizes [INV-30], the pre-show gate reads the same from the styled file about to be shown, run by `scripts/preshow-legibility-lint.py` beside the register lint; the pack ships the law + the default numbers (normal text ≥ 4.5:1, large text (≥ 24px, or ≥ 18.66px bold) ≥ 3:1, body/caption ≥ 12px, a host may set its own) + the script, the browser-computed assertion living in the adopting project's suite (the INV-136 split, live-spec has no UI); register and legibility are the two guards where text meets the eye; homes — this design-principle clause + the ARCHITECTURE frontend design-principles row + communicator's pre-show walk + build-pipeline's verify feel pass + the script + the matrix row | Throwing a wish |
| INV-140 | the prover labels each finding a defect or a recommendation: a defect is a finding where a stated invariant is violated, a spec claim is false, or a required invariant is missing, and it BLOCKS (the design becomes buildable only once it is folded); a recommendation is a finding where nothing stated is broken and nothing required is missing, a consistency or quality gain on offer, and it QUEUES for a taste call (carrying an optional now/later grade when queue order matters); the kind is derivable from the finding's ground (a broken or missing invariant or false claim → defect; only "these siblings should match" or "this could read clearer" with no invariant behind it → recommendation); the push gate folds every defect and queues every recommendation, its rule living at the gate [M-6, INV-30, INV-99] (the one exception: at a delta-scoped gate [INV-114] a pre-existing defect outside the delta queues by that law and never blocks); it names the blocks-or-queues verdict at the point of report rather than leaving the human to sort by hand; homes — this prove-step clause + product-prover's finding tag line, KIND block, Phase 5 partition and record column + the matrix row | Throwing a wish |
| INV-141 | a design review reads a spec the prover has already checked [INV-140] and judges the design behind it: it runs in full at every full prover pass and in a scoped form whenever a new surface is added (the scoped form reading the new surface's elements against the existing inventory), and stands down at feature intake and at the push gate where fit and the gate are the work; it builds its own inventory of every element a person acts on that a spec sentence names, deeper than the page-level surface list down to each photo, caption, control, and slot, this inventory its own transient working list never written into the surface list, which stays the host's own authorship [E-10]; for each element it writes one plain sentence of what the person does with it in the person's own action words rather than any category name, elements whose sentences match proposed as a same-kind group, and within each group it checks whether the members behave the same way (the same gestures, transitions, and affordances); a divergence is a whole behaviour a sibling carries and this member lacks, and where the grouping or the difference is not plain the pass stays silent; every finding names two concrete objects each with its spec sentence, its findings are recommendations or questions to the human and it produces no blocking defects [INV-140] so it can never hold a landing, each run writing a dated record with a per-finding outcome column; where the spec already declares the class its uniformity check governs [INV-125] and the design review reaches the groupings no clause has yet declared; a human confirmation that two elements are the same kind becomes a class sentence written by the spec author naming the class and its members [INV-125] which the existing checks then hold, a human ruling that they differ by intent becomes a decided sentence that closes the question for good [INV-59]; it runs in the project kind's own form [INV-30, INV-136, INV-139] and stands down by name in the record on a kind with no elements a person acts on [INV-22] rather than running vacuously; enumerated at the M-1 milestone gate [M-1]; homes — this prove-step clause + the design-reviewer skill + its ARCHITECTURE design-review node + spec-author's class sentence + the dated design-review record | Throwing a wish |
| INV-142 | every design review finding carries a confidence read of one of two values, confident or likely [INV-141]: a finding is confident when the reviewer would defend the grouping and the divergence on the spec text alone, likely when the deciding fact, whether the two are truly one kind, lives only in the human's intent; a confident finding is written as a recommendation, so it queues for a taste call and never blocks [INV-140]; a likely finding is written as one question to the human with both objects in hand, raised only when the signal is strong (the shared role fits one plain sentence with no stretch, the difference is a whole behaviour one member carries and the other lacks rather than a parameter, and no spec sentence already decides the difference); the question shows both objects each with its spec sentence, asks how alike the two should behave, and carries a recommended default, riding the same batched-question path the pack already uses to bring a judgement to the human [INV-30, E-22, INV-4]; the recommended default is not applied to the spec the way the pack's usual proceed-on-recommended arm would [INV-4] — the class sentence lands only on the human's word [INV-141], while the lane never blocks so the work does not stall; at most three such questions ride per pass, strongest first, a signal below that bar staying silent; an unanswered question is held on the dated record and is not raised again on its own until the human answers it, so it does not re-fire as noise each pass [INV-130] — a stated read, each pass first reading the prior records' open asks and dropping a freshly-derived divergence already carried there; this channel carries exactly one producer for now, the same-kind divergence from the similarity check, a later producer earning its own clause, and like every design review finding it holds no landing [INV-141]; homes — this prove-step clause + the design-reviewer skill's confidence read + communicator's batched-question path + the dated design-review record | Throwing a wish |
| INV-154 | the prover [INV-140] and the design review [INV-141] form a loop over repeated rounds (a round is one prover re-read of the changed part then one design-review re-read of the current spec): only a human-accepted declaration advances the loop — a grouping declared a class sentence [INV-125] or a difference decided a decided sentence [INV-59] is a spec change, so the next round re-reads the re-partitioned elements plus any new element a prover fix added (an element is a thing a person acts on [INV-141]; a declaration adds a grouping over existing elements, a prover fix may add an element); a round producing a new class/decided sentence is a progressing round, while a round's other outputs stay put and do not advance the loop (a confident finding queues as a recommendation for a taste call [INV-142], a likely finding rides as a question the human may answer later [INV-142]); when a round produces no new class/decided sentence the loop rests, its record naming why in one of three ways — it CONVERGES when the design review ran and left no open question and no new grouping to rule on (every grouping already declared, already decided, or standing as a queued recommendation from an earlier round, so a standing recommendation does not bar convergence), it WAITS when the round's findings include a question the human has not answered [INV-142] (the held question resumes the loop when the human answers [INV-59]), and it STANDS DOWN when the design review did not run because the kind has no element a person acts on [INV-141], recorded by name so the rest is not read as coverage it never gave; termination is not guaranteed (a declaration can re-partition into fresh candidate groupings and a prover fix can add elements, so undeclared groupings can rise as well as fall), so convergence is the ordinary case and the cap is the liveness guarantee — three progressing rounds by default (host-settable), the design review counting its own progressing rounds and stopping at the cap (the count belongs to the design-review pass, advances only on a progressing round, persists across the human's asynchronous answers to that pass, and resets when a fresh pass opens); on reaching the cap the loop stops iterating and surfaces on the dated record the unsettled groupings and its best-effort reading of the cause among three (a declaration spawning new elements or groupings faster than it resolves them, an oscillation between two mutually-exclusive groupings, or a self-contradicting spec, best-effort since spec self-consistency is not decidable); surfacing at the cap holds no landing — like every design-review finding it is a recommendation or question [INV-141] and the landing proceeds with the unsettled groupings recorded, the re-prove triggered only by the human's accepted declaration (an ordinary spec edit) and a design-review finding on its own never triggering it or holding a landing [INV-141]; homes — this prove-step clause + build-pipeline's step-2 confirmed-grouping re-entry + the design-review node's loop seam in ARCHITECTURE.md + a matrix row + a guard test | Throwing a wish |
| INV-105 | one canonical state directory named `.live-spec`, stated once and no second directory competing; a near-miss directory (`.livespec`, `.live_spec`, `livespec/`) found at attach or resume is a red finding retired to the attic under a manifest line [INV-7]; worktree isolation is the default when two concurrent lanes' write-sets overlap, the later lane building in its own isolated copy and integrating under the pen [INV-11, T-18, INV-39] | Package repo |
...
| INV-147 | the monitor's law: a single-instance-per-host scheduled process (a second invocation stands down on a lock, a stale lock stolen by age) that surfaces stranger items into committed inbox files and holds no verdict — wish/feedback/neither stays the inbox sweep's call [T-20], a no-wish item closed with a recorded note; a deposit that does not complete is logged and retried, never counted done [INV-67]; the schedule is required where the door is open since the door's liveness rests on it (a stranger cannot sweep itself); two hosts' monitors on one repo converge on a single surfacing by the cross-host claim [INV-149], a maintainer-dropped duplicate remaining only the rare backstop [INV-1]; a run that cannot reach the repo fails honestly [INV-67]; home `scripts/stranger-wish-monitor.py` | Package repo |
...
| INV-148 | the package repo's concrete monitor schedule realizing INV-147: a scheduled GitHub Action `.github/workflows/stranger-monitor.yml` on a daily cron plus manual dispatch that runs the monitor once and pushes any inbox file it committed; single instance by the action's concurrency group, a second run waiting rather than running beside the first (the CI form of the per-host lock [INV-147]); commits and pushes under the action's own token as `github-actions[bot]` with contents plus issues plus discussions write — contents to push the inbox commit, issues and discussions to record the marker comment [INV-146]; the push carries inbox commits only so it rides the inbox-only push-gate carve-out [M-6, INV-112]; a run that cannot reach the repo fails the job honestly [INV-67] | Package repo |
...
| INV-102 | a test's expected value derives independently of the code under test: never recompute the code's own formula and assert it as the expected value, since such an assertion is a mirror that only proves the code equals itself; legal sources of an expected value — a hand-computed constant, an independent derivation, a recorded real output reviewed by a human; boundary — a round-trip or property test over the outputs is legal because it asserts an invariant, the ban strikes only an assertion whose expected value the code's own formula produced | From the spec to the tests |
| INV-103 | every landed queue row carries its delegation accounting in the landed row's status cell — what went to a worker with a rough saving, or a stood-down line naming why the senior kept the work; a suite check reads the queue (ROADMAP.md) and a landed row without the line goes red; binds forward [INV-159] from its landing date (a row landed 2026-07-12 or later carries the line, earlier rows stay stock); the duty binds the orchestrator seat regardless of model; born the night prose alone did not hold the routing rule (2026-07-12) | Who decides what |
| INV-137 | the orchestrator reads to decide, not to discover: the lead's context stays lean by dispatching reads, not performing them — it holds orchestration material (the human's words, the decisions, workers' distilled results, the anchors it must cite), and any read done to understand or design past a glance is dispatched to a reader worker that returns the distillation, the lead never reading raw file bodies; a glance is bounded (one small file, or targeted lines whose result IS the deliverable), past it the read routes like any work [INV-69]; governs the lead's own reading (the input / context-hygiene side) where [INV-69] governs the work it produces and [INV-103] what it reports; made visible by the landing report's delegation accounting naming reads dispatched beside work delegated [INV-103]; homes — base rule 25 + this clause + the delegation accounting [INV-103] + the chat-law hook's routing-line reminder | Who decides what |
| INV-143 | the orchestrator decides what it can decide and reports the choice — a mechanical step, a value a proven artifact already determines [INV-121], a sensible default it can pick and name [INV-70] — surfacing a decision to the human only where it genuinely cannot be made without them (a taste call, a trade-off no artifact settles [INV-121], a change to the definition of correct); it never parks derivable work on the human's queue to avoid deciding [INV-4], and the posture holds on every session including one resumed from its files after a memory wipe [INV-48]; homes — base rulebook rule 27 + this delegation clause | Who decides what |
| INV-152 | a deferral must justify itself: a work item carrying a needs-the-human's-word marker (a held queue row, a NEXT_STEPS line, an open setup-script decision) is re-tested by derivability every time it is touched — an answer pinning to an existing artifact (a base rule, a spec sentence, the architecture, an approved prototype, an already-answered decision [INV-59]) makes the item the seat's (do it, cite the artifact, drop the marker [INV-121, INV-143]); an answer needing a fact no artifact holds (a taste, a policy, an act irreversible outside git [INV-17]) makes it the human's and the marker stands; writing such a marker requires naming that human-only fact, a marker that cannot name it defaulting to the seat's, and a marker that cannot say why the item is the human's being itself the finding, the same shape as a request matching no kind [INV-151]; the no-homeless-item control applied to a work item, kin of the decide-what-it-can-decide posture [INV-143]; enforcement two arms — the mechanical net guardrails/check-deferral-marker.py (a parked item naming no reason category reds the commit) and the chat-law hook's deferral line delivering the re-test at the marker/AskUserQuestion moment [INV-28]; homes — base rulebook rule 29 + this deferral clause | Who decides what |
| INV-109 | a rewrite or restyle that removes substance (a section, an argument, a rationale, a worked example) lists every removal in its landing report, one line of judgment each — the fact kept and where, the owner killed it by name, or the rewriter proposes dropping and asks; a removal the rewriter cannot justify becomes a question before the report closes; never a silent cut of substance; scopes to substance and leaves line-level wording free; homes: communicator's pre-report walk + build-pipeline's docs-only door | Throwing a wish |
| INV-113 | a deliberate architecture redesign re-shapes and re-proves the document in the same movement: when structure is redesigned (layers restacked, a surface's ownership moved, nodes merged or split) ARCHITECTURE.md is re-shaped to the new form and re-proven with the architecture lens in that movement, not only re-pinned; the pins-only path is scoped to a boundary shift that leaves the document's shape standing, since after a real redesign the old shape itself lies and fresh pins on a stale shape are a defect; the re-carve routing [INV-37, E-14] carries the row, this states what the row owes the document; homes: the architecture-doc clause + build-pipeline's architecture step + the refactor line | From the spec to the tests |
| INV-114 | a restructure or migration merge gate judges the delta: three parts — load-bearing token identity old-versus-new modulo the per-chunk named deltas plus the punctuation-multiset check [INV-111], the full suite green on the merged tree [INV-39], and a full prover pass on both sides whose blocking set is delta-scoped (an unmatched token, a red suite, a new-side finding absent on the old side, an unnamed meaning change); pre-existing findings equal on both sides route to queue rows in the same landing and never block; the token-identity part scopes to content-preserving restructures, and a content-changing redesign routes by INV-113; and a session that sharpens a human's spoken bar beyond his words says the sharpened form back and marks it as its own interpretation; homes: the spec clause + product-prover's restructure-merge gate + build-pipeline's restructure door | Catch-up |
...
| INV-155 | a test is green only when it passes deterministically, and a flaky test whose root is in OWNED code is a defect fixed at that root: one question routes a flake — is the nondeterminism removable in code the project owns (test or product)? if yes (wall-clock time, test ordering, shared or leaked state, an unseeded random, a timing or animation assumption, a missing wait on an external tool the test drives) it is a defect, the source named and removed so it passes every run for the same reason, masked by nothing — no retry, no rerun-until-green, no raised timeout that hides the race, no "it passed this time" taken as a pass; if not removable in owned code (the external tool itself misbehaves at random) it is workshop noise on the problem ledger [INV-23], a separate home; the seam question (removable in owned code) decides which home, so an owned defect never drifts into the ledger where a second occurrence could be tolerated as an agreed non-problem [INV-9]; enforcement is two nets — a mechanical gate greps the test config for a retry/rerun-until-green plugin and reds (deterministically detectable), the rest the verify walk's discipline (the fresh-eyes-audit kind of net [INV-46]); where the root is unclear a bounded read of the test and its inputs finds it first (the good infrastructure fix comes first); a flake understood but not removable in one landing is quarantined by name in the pinned skip-set, marked a quarantined flake distinct from an environment skip, with a dated reason and an owning queue row [INV-80, INV-70], holding no landing and standing as a debt the milestone audit reads [INV-145]; the verify-step green definition and the test-author skill both carry it, green meaning deterministic; homes — this clause + build-pipeline step-8 green definition + test-author's determinism rule + INV-23's boundary + a matrix row + the retry-plugin guardrail | From the spec to the tests |
| INV-156 | every review pass records its outcome of one class, so a later session reads every pass the same way: the prover's spec re-check [INV-140], the design review [INV-141], the periodic adversarial audit with its whole-pack skill-creator craft walk [INV-145], and the verify-by-deed audit [INV-46]; three write a dated file of one shared shape under the pass's own home (prover `docs/prover/`, design review `docs/design-review/`, audit `docs/audit/`), opening by naming the skill and version that ran it, a per-finding disposition column, a same-day `-suffix` so two passes in one day never overwrite; the prover and design-review records are the full siblings (prover disposition folded / rejected(+why) with the finding's kind, design-review recommended / asked / answered(+decision) / held, the design review alone holding an ask across passes [INV-142]), the audit record carrying folded / rejected(+why) for a craft walk and MET / OWED / FLAG for a milestone read with no held-ask [INV-145]; the verify-by-deed audit is the one deliberate difference — its verdict and the per-landing skill-creator review it runs [INV-99] land in the landing record's accounting rather than a dated file of this class, verify being a per-landing gate not a periodic pass [INV-46]; the class binds forward [INV-159], a new review pass stating its record as a member or a named difference while pre-declaration records stand unreshaped; never a review pass whose record shape no reader can predict, never verify silently promoted to a dated class file; homes — this records clause + product-prover's record section + design-reviewer's record section + build-pipeline's verify/landing-record accounting | Throwing a wish |
| INV-157 | a browser test harness launches muted (`--mute-audio`, silencing the browser at its source, leaving system volume alone) and reaps the whole process group of the browser it spawned (helper/renderer/gpu children) on teardown AND sweeps its own stale process groups on launch (the crash/interrupt/sleep path that kills a run before teardown [INV-76]) so no orphan accumulates across runs and saturates the machine, and bounds each command with a real per-command deadline (a slow machine waits, a genuine hang fails bounded) rather than a blanket I/O timeout that fires under load and reads a slow machine as a failed test; the net lives where each half is checked — the pack's suite string-checks the shipped template, a consumer's suite asserts by deed with a post-run process-group diff [INV-150]; INV-155's boundary made concrete for the harness — a fault CAUSED by the harness's own run hygiene (an orphan it left, a timeout it set) is removable in owned code and root-fixed (mute/reap/bounded deadline), never masked by retry, while only a fault with nothing to correct in owned code (an external browser crashing at random) is the problem-ledger noise [INV-23]; homes — this clause + INV-100's test-hygiene family (mute + reap, the machine left as found) + INV-155's deadline boundary (no timeout that hides a race) + the pack's shipped harness template + test-author's harness rule + a matrix row | From the spec to the tests |
| INV-158 | the browser test harness is one artifact shipped once by the pack as a template, not a copy each project maintains alone; a consumer adopts it by updating the pack and layers its project-specific driving methods on the shared core, so a core fix (launch flags, teardown, deadline) lands once and reaches every consumer through the update and the implementation cannot drift into divergent copies (a hardening in one, a mute in none); the pack's one-home law [base rule 4] applied to test infrastructure, a project forking a private copy owning the divergence it creates; homes — this clause + the pack's shipped harness template + a matrix row + the catch-up walk [INV-110] and the migration path [INV-91] (the adopt path) | From the spec to the tests |
| INV-159 | a duty binds forward from the first landing that touches its surface after the clause exists, never retroactively across what already landed — an item queued or a project attached before the clause owes no backfill and carries the rule the moment it next moves (its in-work claim its intake), a predating project bringing the rule up as an owned landing; the one statement of the forward-binding convention every such duty cites in place of restating it — the work-kind and pack-intake axes [T-16], the success-measure and lens-sweep duties [INV-21, INV-29], the spec-and-architecture pair and its budgets [INV-15, INV-41, A-3], the runtime and placement views [INV-74, INV-75], scenario edges [INV-127], the norm pointer [E-17], and each self-enforcing landing rule stating its own instance and citing this law [INV-103, INV-134, INV-156, INV-160]; a "binds forward" clause cites this law, a stray one with no cite the standing net's finding [INV-125 kin]; a prototype writes none of it [E-17]; homes — this clause + the forward-binding paragraph in the work-kind section + build-pipeline's lifecycle | Throwing a wish |
| INV-160 | the suite-honesty / test-infrastructure invariants are one declared class — INV-77, INV-78, INV-79, INV-80, INV-100, INV-102, INV-155, INV-157, INV-158 — each closing a way the suite could pass green while its claimed fact is false or leaving the machine worse than found; one parity, each member naming its net past naming the fix — a mechanical run-reddening check for eight members, and for the boundary member INV-77 a real-device walk row the suite can never green (the assertion shape for INV-77/78/79, the independent expected-value source for INV-102, the temp-home diff for INV-100, the three plumbing legs for INV-80, the retry-grep plus quarantine for INV-155, the template string-check plus the consumer's by-deed process-group diff for INV-157, the one-home template assertion for INV-158); a member naming no net is a class defect the prover blocks [INV-125], the standing an under-enumerated review-record member has [INV-156]; binds forward [INV-159], pre-declaration members unreshaped; homes — this class clause + the test-infra section + product-prover's declared-class lens [INV-125] + test-author | From the spec to the tests |
