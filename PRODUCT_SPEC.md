# live-spec — Product Spec (v2.4.0, 2026-07-17)
...
| INV-138 | A behaviour gated on a ranged quantity states both ends of the range, and an async-filled slot names its pending, arrived, and failed faces with a visible pending face [INV-72]; a layout guarantee names its viewport quantifier, holding on every viewport or naming the band it is scoped to, and the prover draws from a band-scoped guarantee the standing question about the other bands [INV-141, INV-150]. | Composing across axes |
