...
|---|---|---|---|
| full suite wall-time | ≤ 420 s on the dev machine [default] (measured 383–405 s serial at 1,856 tests with the suite-in-suite meta-test firing, 2026-07-24 at the row-456 architecture-format landing — the format member's own tests grew the suite from 1,809, so the budget row moved with the fresh measured figure; a diff outside the gate-machinery class skips the meta-test and runs well under) | the pytest tail line in the suite run's log, read by `guardrails/check-suite-budget.sh` on every full gate run and red past the budget naming both figures (M-346, row 361) | `guardrails/check-suite-budget.sh` reds past the budget on every full gate run, naming both figures (M-346) |
