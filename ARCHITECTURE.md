...
| build-pipeline | the wish lifecycle: intake → classify → spec → prove → architecture → matrix → test → code → land | E-2, T-1..T-6, T-8, T-9, T-11, T-12, T-15, T-16, T-17, INV-1, INV-2, INV-3, INV-4, INV-12, INV-16, INV-22, INV-26, E-14, E-15, INV-15, M-1 | `skills/build-pipeline/SKILL.md:32` (step zero: the door + work-kind), `:69` (the work-kind table — per-kind meanings' one home), `:93` (steps), `:204` (gates) |
| communicator | the human-facing exchange: reports, batched questions, decision pages, done-claim answers, the capture echo + departures board | T-7 (the report step; the walk before it is build-pipeline's), E-22, INV-25, INV-27, INV-28 | `skills/communicator/SKILL.md:33` (the rules), `:102` (rule 10 — the decision page), `:122` (rule 11 — the evidence walk), `:100` (rule 9 — the outcome-leads line shape) |
