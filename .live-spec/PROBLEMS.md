...
|---|---|---|---|
| zsh eats a bare `===` echo separator | 2026-07-05 (session 7), 2026-07-05 (session 9), 2026-07-05 ~23:55 (session 11, fired once more before the verdict was read), 2026-07-06 ~10:15 (session 12 — fired again mid-work; the hand typed it a fourth time) | OWNED 2026-07-06 ~10:30 [Alexander's word 2026-07-06 morning: decision DELEGATED («проанализируй взаимозависимости и реши»). The delegated verdict: interdependencies none — my own terminal habit, nothing in product or method touches it — but four recurrences prove discipline lost; same medicine as the invented-time entries: a MECHANICAL fence. Installer ready at `scripts/install-separator-fence.sh` (a PreToolUse hook that blocks a bare `===` in my commands, self-testing); the harness classifier forbids me editing my own hook config, so it takes Alexander's one run: `! sh ~/live-spec/scripts/install-separator-fence.sh` — flips to SOLVED then] | shell glob behaviour, not ours to fix; standing workaround: separators are `---`, never bare `===` |
